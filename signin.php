...
$_POST['last_name'] = trim($_POST['last_name']);
	try {
		...
		if(strlen($_POST['mobile']) != 10 && preg_match("/^\d+$/",$_POST['mobile'])){
			throw new Exception('Invalid Mobile Format');
		}
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >1 ) {
						...
						$result = mysqli_query($con, "INSERT INTO user (firstName,lastName,email,mobile,address,password,confirmCode) VALUES ('$_POST[first_name]','$_POST[last_name]','$_POST[email]','$_POST[mobile]','$_POST[signupaddress]','$_POST[password]','0')");
